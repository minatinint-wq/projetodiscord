...
export const api = {
  ...
  acceptFriend: id => request(`/api/friends/${id}/accept`, { method: 'POST' }),
  removeFriend: id => request(`/api/friends/${id}`, { method: 'DELETE' }),
  servers: () => request('/api/servers'),
  createServer: (name, options = {}) => request('/api/servers', { method: 'POST', body: JSON.stringify({ name, icon: options.icon, template: options.template }) }),
  ...
  updateServer: (serverId, input) => request(`/api/servers/${serverId}`, { method: 'PATCH', body: JSON.stringify(input) }),
