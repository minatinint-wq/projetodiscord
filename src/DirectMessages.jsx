...
export default function DirectMessages({user,currentUser,onClose,onOpenProfile,onJoinVoice,Avatar}){
 ...
 <div className={"direct-main file-drop-zone"+(drop.dragging?" dragging":"")} {...drop.bind}>{drop.dragging&&<div className="file-drop-overlay"><strong>Solte o arquivo aqui</strong><span>Até 3 MB · você confirma antes de enviar</span></div>}<div className="direct-messages" aria-live="polite"><div className="direct-welcome"><Avatar user={user}/><h2><StyledName user={user}/></h2><p>Uma conversa só entre vocês.</p></div>{loading&&<p>Carregando conversa…</p>}{messages.map(message=><article className="dm-message" key={message.id}><Avatar user={message.author} small/><div><header><StyledName user={message.author}/><time dateTime={message.createdAt}>{new Date(message.createdAt).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}</time></header><p>{message.content}</p>{message.attachment&&<AttachmentView attachment={message.attachment} alt="Imagem enviada na conversa"/>}</div></article>)}<div ref={bottom}/></div>
 ...
 <aside className="direct-profile"><div className="direct-profile-banner" style={bannerPresentation(user)}/><Avatar user={user}/><h2><StyledName user={user}/></h2><p>{user.bio||"Diga oi e comece a conversa."}</p><h3>Jogos em destaque</h3><div className="profile-game-chips">{(user.gameInterests||[]).map(id=>GAME_CATALOG.find(game=>game.id===id)).filter(Boolean).map(game=><span key={game.id}><GameIcon game={game}/>{game.name}</span>)}</div><button className="direct-profile-link" onClick={onOpenProfile}>Ver perfil completo</button></aside>
 </section>;
}
