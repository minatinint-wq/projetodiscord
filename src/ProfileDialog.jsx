...
export default function ProfileDialog({data,currentUser,onClose,onRetry,onEdit,onMessage,onAddFriend,onVoice,Avatar,ProfileEffectLayer,renderBadges,presence,isFriend,serverRole}){
 ...
 {!!serverRole&&<section className="identity-section"><h3>CARGO NESTA COMUNIDADE</h3><div className="identity-role-list"><span className={"identity-role role-style-"+(serverRole.style||"solid")} style={{"--role-color":serverRole.color,"--member-role-color":serverRole.color}}>{serverRole.icon?<img src={serverRole.icon} alt=""/>:serverRole.emoji?<span className="role-emoji" aria-hidden="true">{serverRole.emoji}</span>:<i/>}<span className="role-effect-text">{serverRole.name}</span></span></div></section>}
