...
export function StyledName({user,children,withPlate=false}) {
 const role=user?.serverRole;
 const roleClass=role?" role-effect-text role-style-"+(role.style||"solid"):"";
 const plate=withPlate?nameplateSrc(user?.profilePlate):null;
 const name=<span className={"styled-name name-effect-"+(user?.nameEffect||"solid")+" font-"+(user?.nameStyle||"default")+roleClass} style={{"--name-color":user?.nameColor||"#f1f3f5","--member-role-color":role?.color||"#f1f3f5"}}>{children||user?.displayName||user?.username}</span>;
 return plate?<span className="nameplate-name"><video src={plate} autoPlay loop muted playsInline preload="metadata" aria-hidden="true"/>{name}</span>:name;
}
