...
test("Nitro aplica banner, moldura e sobreposição premium com animações internas", async ({ page, browser }) => {
  ...
  try {
    ...
    await expect(overlayMotion).toHaveAttribute("src",/cosmetics-animated\/overlays\/steel-wolf\.webp$/)
    ...
    await expect(avatarMotion).toHaveAttribute("src",/cosmetics-animated\/frames\/steel-wolf\.webp$/)
    ...
    await expect(bannerMotion).toHaveAttribute("src",/cosmetics-animated\/banners\/steel-wolf\.webp$/)
