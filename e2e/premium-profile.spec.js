...
test("Nitro aplica banner, moldura e sobreposição premium com animações internas", async ({ page, browser }) => {
  ...
  try {
    ...
    await expect(overlayMotion).toHaveAttribute("data-animated-cosmetic",/cosmetics-animated\/overlays\/steel-wolf\.sprite\.webp\?v=5$/)
    const avatarMotion=editor.locator(".profile-editor-preview .premium-avatar-frame-scene-steel-wolf .premium-avatar-frame-motion")
    await expect(avatarMotion).toBeVisible()
    await expect(avatarMotion).toHaveAttribute("data-animated-cosmetic",/cosmetics-animated\/frames\/steel-wolf\.sprite\.webp\?v=5$/)
    const captureFrame=locator=>locator.evaluate(canvas=>canvas.toDataURL())
    const decodedFrameA=await captureFrame(avatarMotion)
    await page.waitForTimeout(260)
    const decodedFrameB=await captureFrame(avatarMotion)
    expect(decodedFrameB).not.toBe(decodedFrameA)
    ...
    await expect(bannerMotion).toHaveAttribute("data-animated-cosmetic",/cosmetics-animated\/banners\/steel-wolf\.sprite\.webp\?v=5$/)
