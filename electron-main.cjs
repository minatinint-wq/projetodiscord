...
function startBackend() {
  ...
  backend.on('error', error => dialog.showErrorBox('Orbit', `Não foi possível iniciar o servidor local: ${error.message}`));
...
function waitForBackend(attempt = 0) {
  if (attempt > 30) return dialog.showErrorBox('Orbit', 'O servidor local não respondeu.');
...
function createWindow() {
  window = new BrowserWindow({ width: 1400, height: 900, minWidth: 900, minHeight: 600, backgroundColor: '#0c101a', title: 'Orbit', webPreferences: { contextIsolation: true, nodeIntegration: false } });
  waitForBackend();
}
